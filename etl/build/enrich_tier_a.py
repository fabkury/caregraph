...
derived metrics into existing entity manifests:
  ...
  6. enrich_counties_chronic — Medicare chronic condition prevalence/spending
  7. enrich_drugs_nadac — NADAC drug acquisition costs
...
def _load_chronic_conditions(
    csv_path: Path,
) -> dict[str, dict[str, Any]]:
    """Load Medicare Specific Chronic Conditions by County data grouped by FIPS.

    Filters to county-level rows (Bene_Geo_Lvl == "County").

    Returns {fips: {year, conditions: {condition_name: {prevalence, spending_per_capita}}}}.
    """
    result: dict[str, dict[str, Any]] = {}
    rows = _read_csv(csv_path)
    if not rows:
    ...
    col_geo_lvl = _find_column(sample, [
        "Bene_Geo_Lvl", "bene_geo_lvl", "Geographic Level",
        "Bene_Geo_Level",
    ])
    col_geo_cd = _find_column(sample, [
        "Bene_Geo_Cd", "bene_geo_cd", "Geographic Code",
        "FIPS", "County_FIPS",
    ])
    col_condition = _find_column(sample, [
        "Bene_Cond", "bene_cond", "Condition",
        "Chronic_Condition", "Condition_Name",
    ])
    col_prevalence = _find_column(sample, [
        "Prvlnc", "prvlnc", "Prevalence",
        "Bene_Cond_Prvlnc", "Prevalence_Rate",
    ])
    col_spending = _find_column(sample, [
        "Tot_Mdcr_Stdzd_Pymt_PC", "tot_mdcr_stdzd_pymt_pc",
        "Per_Capita_Spending", "Spending_Per_Capita",
        "Mdcr_Stdzd_Pymt_PC",
    ])
    col_year = _find_column(sample, [
        "Bene_Geo_Desc", "Year", "year",
        "Data_Year", "Bene_Age_Lvl",
    ])

    if col_geo_cd is None:
        print("    [warn] Could not find geographic code column in chronic conditions data")
        print(f"    [warn] Available columns: {list(sample.keys())[:15]}...")
        # Return empty — function is robust to missing data
        return result

    print(f"    [chronic] Geo column: '{col_geo_cd}', Condition: '{col_condition}'")

    county_rows = 0
    fips_conditions: dict[str, dict[str, dict[str, Any]]] = defaultdict(dict)
    year_seen = ""

    for row in rows:
        # Filter to county level if the geo-level column exists
        if col_geo_lvl:
            geo_lvl = _clean(row.get(col_geo_lvl, ""))
            if geo_lvl and geo_lvl.lower() != "county":
                continue

        raw_fips = row.get(col_geo_cd, "")
        fips = normalize_fips(raw_fips)
        if fips is None or len(fips) != 5:
            continue

        condition = _clean(row.get(col_condition, "")) if col_condition else ""
        if not condition:
            continue

        prevalence = _try_float(row.get(col_prevalence, "")) if col_prevalence else None
        spending = _try_float(row.get(col_spending, "")) if col_spending else None

        if prevalence is None and spending is None:
            continue

        cond_data: dict[str, Any] = {}
        if prevalence is not None:
            cond_data["prevalence"] = round(prevalence, 2)
        if spending is not None:
            cond_data["spending_per_capita"] = round(spending, 0)

        fips_conditions[fips][condition] = cond_data
        county_rows += 1

        if not year_seen and col_year:
            raw_year = _clean(row.get(col_year, ""))
            match = re.search(r"(20\d{2})", raw_year)
            if match:
                year_seen = match.group(1)

    # Build final structure
    for fips, conditions in fips_conditions.items():
        result[fips] = {
            "year": year_seen or "2018",
            "conditions": conditions,
        }

    print(f"    [chronic] {county_rows:,} county-condition rows across {len(result):,} counties")
    return result


def enrich_counties_chronic(
    county_dir: Path,
    chronic_csv: Path,
    download_date: date,
) -> int:
    """Enrich county manifests with Medicare chronic condition data.

    Returns the number of manifests enriched.
    """
    if not county_dir.exists():
        print("  [enrich-chronic] No county directory found")
        return 0

    print("  [enrich-chronic] Loading chronic conditions data...")
    chronic_data = _load_chronic_conditions(chronic_csv)
    print(f"  [enrich-chronic] Chronic: {len(chronic_data):,} counties with condition data")

    if not chronic_data:
        print("  [enrich-chronic] No chronic condition data available (may be expected)")
        return 0
    ...
    for manifest_path in sorted(county_dir.glob("*.json")):
        ...
        fips = manifest.get("fips", "")
        if fips not in chronic_data:
            continue

        manifest.setdefault("data", {})["chronic_conditions"] = chronic_data[fips]

        provenance_list = manifest.get("provenance", [])
        existing_ids = {p.get("dataset_id") for p in provenance_list}
        if "cms-chronic-conditions" not in existing_ids:
            provenance_list.append(
                build_provenance(
                    dataset_id="cms-chronic-conditions",
                    dataset_name="Medicare Specific Chronic Conditions by County",
                    vintage=chronic_data[fips].get("year", "2018"),
                    download_date=download_date,
                    row_count=len(chronic_data),
                )
            )
            manifest["provenance"] = provenance_list
    ...
    print(f"  [enrich-chronic] Enriched {enriched:,} county manifests")
    return enriched
